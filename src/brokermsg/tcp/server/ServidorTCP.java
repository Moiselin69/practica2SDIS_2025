...
public class ServidorTCP implements Runnable {
    ...
    public ServidorTCP(int numHilos, int puertoAbrir) throws IOException {
        this.numHilos = numHilos;
        this.puertoAbrir = puertoAbrir;
        socketServidor = new ServerSocket(this.puertoAbrir);
        try{
            usuariosHashMap = mapper.readValue(new File("src/sdis/config/usuariosContras.json"),
                                                new TypeReference<ConcurrentHashMap<String, String>>() {});
            ConcurrentHashMap<String, ConcurrentLinkedQueue<String>> temp = mapper.readValue(new File("src/sdis/config/colasMensajeria.json"),
                                                new TypeReference<ConcurrentHashMap<String, ConcurrentLinkedQueue<String>>>(){});
            mapaMensajesAddRead = mapper.readValue(new File("src/sdis/config/depuracionColasMensajeria.json"),
